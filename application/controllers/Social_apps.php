...
class Social_apps extends Home
{
    ...
    public function facebook_settings_data()
    {
        ...
        foreach ($info as $key => $value) 
        {
            ...
            if($this->is_demo != '1')
            $info[$i]['action'] .= "<div class='min_width_130px'><a href='".base_url('social_apps/edit_facebook_settings/').$value['id']."' class='btn btn-outline-warning btn-circle' data-toggle='tooltip' data-placement='top' title='".$this->lang->line('Edit APP Settings')."'><i class='fas fa-edit'></i></a> ";
            ...
            $info[$i]["action"] .="<script src='".base_url()."assets/js/system/tooltip_popover.js'></script>";
            $i++;
        }
