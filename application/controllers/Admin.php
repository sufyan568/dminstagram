...
class Admin extends Home
{
    ...
    public function analytics_settings_action()
    {
        ...
        $google_id = addslashes(strip_tags($this->input->post('google_id',true)));

        $app_package_config_data = "<?php ";
        $app_package_config_data.= "\n\$config['pixel_id'] = '$pixel_id';\n";
        $app_package_config_data.= "\$config['google_id'] = '$google_id';\n";
        file_put_contents(APPPATH.'config/analytics_config.php', $app_package_config_data, LOCK_EX);
        $this->session->set_flashdata('success_message', 1);
    ...
    {           
        ...
        foreach ($info as $key => $value) 
        {
            ...
            $info[$i]["actions"] .= "<script src='".base_url()."assets/js/system/tooltip_popover.js'></script>";
            ...
            $info[$i]['name'] = "<span data-toggle='tooltip' title='".$this->lang->line($info[$i]['user_type'])."'><i class='fas fa-user".$tie." text-warning'></i> ".$info[$i]['name']." </span><script src='".base_url()."assets/js/system/tooltip_popover.js'></script>";
