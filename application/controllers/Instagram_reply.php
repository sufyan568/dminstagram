...
class Instagram_reply extends Home
{
    ...
    public function get_account_details()
    {
      if($this->session->userdata('user_type') != 'Admin' && !in_array(279,$this->module_access)) exit;
        ...
        $autoreply_templates = "";
        $template_info = $this->basic->get_data('instagram_reply_template',['where'=>['user_id'=>$this->user_id]]);
    ...
    public function ajax_autoreply_submit()
    {
      if($this->session->userdata('user_type') != 'Admin' && !in_array(279,$this->module_access)) {
        ...
        if ($message_type == 'generic') {
          ...
          {
            $template_sql = "INSERT INTO instagram_reply_template (user_id,auto_reply_campaign_name,reply_type,hide_comment_after_comment_reply,is_delete_offensive,offensive_words,multiple_reply,auto_reply_text,last_updated_at,nofilter_word_found_text) VALUES ('$this->user_id','$auto_campaign_name','$message_type','$hide_comment_after_comment_reply','$is_delete_offensive',$offensive_words,'$multiple_reply',$auto_reply_text,'$date_time',$nofilter_word_found_text
              )
              ON DUPLICATE KEY UPDATE auto_reply_text=$auto_reply_text,reply_type='$message_type',hide_comment_after_comment_reply='$hide_comment_after_comment_reply',is_delete_offensive='$is_delete_offensive',offensive_words=$offensive_words,multiple_reply='$multiple_reply',auto_reply_campaign_name='$auto_campaign_name',nofilter_word_found_text=$nofilter_word_found_text";
    ...
    public function autoreply_template_manager_data()
    {
        ...
        $search_value = $_POST['search']['value'];
        $display_columns = array("#",'id','auto_reply_campaign_name');
        $search_columns = array('auto_reply_campaign_name');
