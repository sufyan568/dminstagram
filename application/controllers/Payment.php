...
class Payment extends Home
{
    ...
    {     
        ...
        if($this->is_demo == '1')
            $data["xvalue"]["stripe_secret_key"]=$data["xvalue"]["stripe_publishable_key"]=$data["xvalue"]["paypal_email"]=$data["xvalue"]["paystack_secret_key"]=$data["xvalue"]["paystack_public_key"]=$data["xvalue"]["razorpay_key_id"]=$data["xvalue"]["razorpay_key_secret"]=$data["xvalue"]["mollie_api_key"]=$data["xvalue"]["mercadopago_public_key"]=$data["xvalue"]["mercadopago_public_key"]=$data["xvalue"]["sslcommerz_store_id"]=$data["xvalue"]["sslcommerz_store_password"]=$data["xvalue"]["senangpay_merchent_id"]=$data["xvalue"]["senangpay_secret_key"]=$data["xvalue"]["instamojo_api_key"]=$data["xvalue"]["instamojo_auth_token"]=$data["xvalue"]["xendit_secret_api_key"]="XXXXXXXXXX";
        $paypal_stripe_currency_list = $this->paypal_stripe_currency_list();
        $marcadopago_country = array('ar'=>'Argentina','br'=>'Brazil','co'=>'Colombia','mx'=>'Mexico');
    ...
    {      
        ...
        if ($this->session->userdata('license_type') == 'double' && $this->session->userdata('user_type') == 'Member') 
        {
            ...
            $pp_button = $st_button = $razorpay_button = $paystack_button = $mollie_button = $mercadopago = $sslcommerz = $senangpay = $instamojo = $xendit ="";
            ...
            if($mercadopago_public_key!='' && $mercadopago_access_token!='')
            {
                ...
                $this->mercadopago->redirect_url=base_url("stripe_action/mercadopago_action");
        ...
        {
            $redirect_url_instamojo = base_url('stripe_action/instamojo_action/').$package_id;
        ...
        {
            $xendit_redirect_url = base_url('stripe_action/xendit_action/').$package_id;
